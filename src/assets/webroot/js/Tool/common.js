function edit_debt_value(formTag){

	$('#loading').show();
	var form_id = formTag.id;
	$.ajax({
		type: 'POST',  
		url: getWebroot()+"Tools/edit_debt_value",  
...
function reset_form(formParent){

	var formTag = formParent.find('form');
	formTag.find('input, select').each(function(){
		$(this).val('');
	});
	formTag.find('label.error').each(function(){
		$(this).hide();
	});
}
...
function update_debt_crusher_common_section(){
	...
	// Calculate Large Month Number
	var large_month = this_month = 0;
	$('input.each_new_saved_month').each(function(){
		this_month = Number($(this).val());
		if (large_month < this_month) {
			large_month = this_month;
		}
	});

	// Update Time saved
	var final_saved_month = large_month;
	if (final_saved_month <= 11) {
		var convert_month_to_year = final_saved_month+" month(s)";
		var convert_month_to_name = final_saved_month+" month(s)";
	}else{
