...
@Component({
  selector: 'app-add-user-client',
  templateUrl: './add-user-client.component.html',
  styleUrls: ['./add-user-client.component.scss']
})
export class AddUserClientComponent implements OnInit {
  ...
  formData: any;
  constructor(
    public dialogRef: MatDialogRef<AddUserClientComponent>,
    private rt: Router,
  ...
  addUser() {
    ...
    console.log(this.adduserForm.value)
    this.clientService.Post(this.addUsers, this.adduserForm.value).subscribe(res => {
      this.common = res
  ...
  getCompany() {
    ...
    this.clientService.Post(this.getCompanies, { token: 'LIVESITE' }).subscribe(res => {
      this.common = res
      console.log(res)
  ...
  getClient(value) {
    this.clientService.Post(this.getClients, { company_id: value, token: 'LIVESITE' }).subscribe(res => {
      this.common = res;
